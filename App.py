...
st.set_page_config(page_title='Resume Analyzer', page_icon='Logo/resume_icon.jpg')
...
def run():
    ...
    if choice == 'User':
        ...
        # Login fields only for Admin
        ad_user = st.text_input("Username")
        ad_password = st.text_input("Password", type='password')
        if st.button('Login'):
            if ad_user == 'user' and ad_password == 'pass':
                ...
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page',
                                                 'Predicted Field', 'User_Level', 'Actual Skills', 'Recommended Skills',
                                                 'Recommended Course'])
                st.dataframe(df)
